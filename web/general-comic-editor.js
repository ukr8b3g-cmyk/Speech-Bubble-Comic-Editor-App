(function (root) {
  ...
  function create(options = {}) {
    ...
    function attachBlob(metadata, blob, render = true) { return imageFromBlob(blob).then(({ image, url }) => { const old = runtimeImages.get(metadata.id)?.dataset?.generalComicObjectUrl; if (old) { URL.revokeObjectURL(old); objectUrls.delete(old); } image.dataset.generalComicObjectUrl = url; objectUrls.add(url); runtimeImages.set(metadata.id, image); runtimeBlobs.set(metadata.id, blob); metadata.width = image.naturalWidth; metadata.height = image.naturalHeight; if (render) { renderTray(); requestRender({ canvas: true, layers: true }); } }); }
    async function hydrateImages() { const target = documentId(); hydratedDocumentId = target; await Promise.all(comic.images.filter((item) => !runtimeImages.has(item.id)).map(async (metadata) => { try { const blob = await loadImageBlob(target, metadata.id); if (blob && target === hydratedDocumentId) await attachBlob(metadata, blob, false); } catch (error) { console.warn("General comic image restore failed", metadata.id, error); } })); renderTray(); requestRender({ canvas: true, layers: true }); }
    ...
    async function importFiles(files) { const valid = files.filter(supportedImage); if (!valid.length) { options.setStatus?.(tr("PNG / JPEG / WebPを選択してください", "Choose PNG, JPEG, or WebP"), "error"); return []; } if (comic.images.length + valid.length > MAX_IMAGES) { options.setStatus?.(tr("ページ画像は100枚までです", "Up to 100 page images are supported"), "error"); return []; } const ids = []; for (const file of valid) { if (file.size > MAX_IMAGE_BYTES) { options.setStatus?.(tr("画像が大きすぎます", "The image is too large"), "error"); continue; } const metadata = { id: `${IMAGE_PREFIX}${uuid()}`, name: String(file.name || "image").replace(/\.[^.]+$/, ""), mime: file.type || "image/png", width: 1, height: 1, sha256: "", source: "stored" }; await attachBlob(metadata, file, false); comic.images.push(metadata); await storeImageBlob(documentId(), metadata, file); ids.push(metadata.id); } if (ids.length) { used = true; selectedTrayImageId = ids.at(-1); changed(); renderTray(); } return ids; }
    ...
    async function importProjectImages(records) { for (const record of Array.isArray(records) ? records : []) { if (!record?.id || !String(record.data_url || "").startsWith("data:image/")) continue; const blob = await fetch(record.data_url).then((response) => response.blob()); let metadata = comic.images.find((item) => item.id === record.id); if (!metadata) { metadata = { id: String(record.id), name: String(record.name || "project-image"), mime: record.mime || blob.type || "image/png", width: 1, height: 1, sha256: "", source: "stored" }; comic.images.push(metadata); } await attachBlob(metadata, blob, false); await storeImageBlob(documentId(), metadata, blob); } renderTray(); requestRender({ canvas: true, layers: true }); }
