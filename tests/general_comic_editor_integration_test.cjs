...
assert.match(html, /generalRecords=records\.filter\(record=>generalIds\.has/);
assert.match(generalEditor, /function importExternalImage\(blob, asset, control = \{\}\)/);
assert.match(generalEditor, /function removeAssetUsage\(imageId, control = \{\}\)/);
